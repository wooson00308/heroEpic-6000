    ...
    void OnSceneGUI()
    {
        ...
        foreach(Collider2D col in hbr.Collider)
        {
            if(col is CircleCollider2D)
            {
                ...
                var fmh_153_62_638651933970786676 = Quaternion.identity; Vector3 center = Handles.FreeMoveHandle(pos, handleRadius/2, handleSize/2, Handles.DotHandleCap);
                float snap = 0.1f;
            ...
            else if(col is BoxCollider2D)
            {
                ...
                var fmh_219_62_638651933970799400 = Quaternion.identity; Vector3 center = Handles.FreeMoveHandle(pos, handleRadius/2, handleSize/2, Handles.DotHandleCap);
                box.transform.position = center;

                Vector3 topLeftPrev = pos + new Vector3(-size.x/2, size.y/2, 0);
                var fmh_223_71_638651933970802700 = Quaternion.identity; Vector3 topLeft = Handles.FreeMoveHandle(topLeftPrev, handleRadius, handleSize, Handles.SphereHandleCap);

                Vector3 topRightPrev = pos + new Vector3(size.x/2, size.y/2, 0);
                var fmh_226_73_638651933970805622 = Quaternion.identity; Vector3 topRight = Handles.FreeMoveHandle(topRightPrev, handleRadius, handleSize, Handles.SphereHandleCap);

                Vector3 bottomLeftPrev = pos + new Vector3(-size.x/2, -size.y/2, 0);
                var fmh_229_77_638651933970808317 = Quaternion.identity; Vector3 bottomLeft = Handles.FreeMoveHandle(bottomLeftPrev, handleRadius, handleSize, Handles.SphereHandleCap);

                Vector3 bottomRightPrev = pos + new Vector3(size.x/2, -size.y/2, 0);
                var fmh_232_79_638651933970811867 = Quaternion.identity; Vector3 bottomRight = Handles.FreeMoveHandle(bottomRightPrev, handleRadius, handleSize, Handles.SphereHandleCap);
